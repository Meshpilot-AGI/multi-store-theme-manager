 ...
 *
 * Path `/auth/mokshya/install?shop=<shop>.myshopify.com` starts the OAuth flow
 * using the MOKSHYA_CLIENT_ID / MOKSHYA_CLIENT_SECRET from .env.
 *
 * Path `/auth/mokshya/callback?code=...&shop=...` completes the flow,
...
const CLIENT_ID = process.env.MOKSHYA_CLIENT_ID;
const CLIENT_SECRET = process.env.MOKSHYA_CLIENT_SECRET;
const SCOPES = process.env.MOKSHYA_SCOPES || "";
...
export const loader = async ({ request, params }) => {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    return new Response("Mokshya OAuth not configured (missing env vars)", { status: 500 });
  }
  ...
  if (sub === "install") {
    ...
    const redirectUri = `${APP_URL}/auth/mokshya/callback`;
  ...
  return new Response("Unknown subpath. Use /auth/mokshya/install or /auth/mokshya/callback", { status: 404 });
};
