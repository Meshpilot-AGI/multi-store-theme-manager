 ...
 *
 * Path `/auth/trendsetters/install?shop=<shop>.myshopify.com` starts the OAuth flow
 * using the TRENDSETTERS_CLIENT_ID / TRENDSETTERS_CLIENT_SECRET from .env.
 *
 * Path `/auth/trendsetters/callback?code=...&shop=...` completes the flow,
...
const CLIENT_ID = process.env.TRENDSETTERS_CLIENT_ID;
const CLIENT_SECRET = process.env.TRENDSETTERS_CLIENT_SECRET;
const SCOPES = process.env.TRENDSETTERS_SCOPES || "";
...
export const loader = async ({ request, params }) => {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    return new Response("Trendsetters OAuth not configured (missing env vars)", { status: 500 });
  }
  ...
  if (sub === "install") {
    ...
    const redirectUri = `${APP_URL}/auth/trendsetters/callback`;
  ...
  return new Response("Unknown subpath. Use /auth/trendsetters/install or /auth/trendsetters/callback", { status: 404 });
};
