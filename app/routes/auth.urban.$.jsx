 ...
 *
 * Path `/auth/urban/install?shop=<shop>.myshopify.com` starts the OAuth flow
 * using the URBAN_CLIENT_ID / URBAN_CLIENT_SECRET from .env.
 *
 * Path `/auth/urban/callback?code=...&shop=...` completes the flow,
...
const CLIENT_ID = process.env.URBAN_CLIENT_ID;
const CLIENT_SECRET = process.env.URBAN_CLIENT_SECRET;
const SCOPES = process.env.URBAN_SCOPES || "";
...
export const loader = async ({ request, params }) => {
  ...
  if (sub === "install") {
    ...
    const redirectUri = `${APP_URL}/auth/urban/callback`;
  ...
  return new Response("Unknown subpath. Use /auth/urban/install or /auth/urban/callback", { status: 404 });
};
