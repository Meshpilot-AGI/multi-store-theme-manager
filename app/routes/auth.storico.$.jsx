 ...
 *
 * Path `/auth/storico/install?shop=<shop>.myshopify.com` starts the OAuth flow
 * using the STORICO_CLIENT_ID / STORICO_CLIENT_SECRET from .env.
 *
 * Path `/auth/storico/callback?code=...&shop=...` completes the flow,
...
const CLIENT_ID = process.env.STORICO_CLIENT_ID;
const CLIENT_SECRET = process.env.STORICO_CLIENT_SECRET;
const SCOPES = process.env.STORICO_SCOPES || "";
...
export const loader = async ({ request, params }) => {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    return new Response("Storico OAuth not configured (missing env vars)", { status: 500 });
  }
  ...
  if (sub === "install") {
    ...
    const redirectUri = `${APP_URL}/auth/storico/callback`;
  ...
  return new Response("Unknown subpath. Use /auth/storico/install or /auth/storico/callback", { status: 404 });
};
