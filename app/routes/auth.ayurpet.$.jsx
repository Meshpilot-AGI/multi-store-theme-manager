/**
 * OAuth handler for the "ayurpet" Shopify Custom App.
 *
 * Installs on `2684sq-mt.myshopify.com`.
 *
 * Path `/auth/ayurpet/install?shop=<shop>.myshopify.com` starts OAuth using
 * AYURPET_CLIENT_ID / AYURPET_CLIENT_SECRET from .env.
 *
 * Path `/auth/ayurpet/callback?code=...&shop=...` completes the flow,
 * exchanges the authorization code for an offline access token, and stores
 * it in the Prisma Session table as `offline_<shop>`.
 */
...
const CLIENT_ID = process.env.AYURPET_CLIENT_ID;
const CLIENT_SECRET = process.env.AYURPET_CLIENT_SECRET;
const SCOPES = process.env.AYURPET_SCOPES || "";
...
export const loader = async ({ request, params }) => {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    return new Response("Ayurpet OAuth not configured (missing env vars)", { status: 500 });
  }
  ...
  if (sub === "install") {
    ...
    const redirectUri = `${APP_URL}/auth/ayurpet/callback`;
  ...
  return new Response("Unknown subpath. Use /auth/ayurpet/install or /auth/ayurpet/callback", { status: 404 });
};
